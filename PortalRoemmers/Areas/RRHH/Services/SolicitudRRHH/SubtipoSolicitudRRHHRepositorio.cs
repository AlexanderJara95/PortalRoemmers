...
{
    public class SubtipoSolicitudRRHHRepositorio
    {
        ...
        public ViewModels.IndexViewModel obtenerTodos(int pagina, string search, string subtipo, string primero, string actual)
        {
            ...
            {
                //SOLICITUDES PROPIAS
                var model = db.tb_SubtipoSolicitudRRHH
                    .Include(x => x.tipoSolicitud.idTipoSolicitudRrhh)
                    .OrderByDescending(x => x.idSubTipoSolicitudRrhh)
                    .Where(x => x.descSubtipoSolicitud.Contains(search))
                    .Skip((pagina - 1) * cantidadRegistrosPorPagina)
                    .Take(cantidadRegistrosPorPagina).ToList();

                var totalDeRegistros = model.Count;
                var modelo = new ViewModels.IndexViewModel();
                modelo.SubTipoSoliRRHH = model;
        ...
        public List<SubtipoSolicitudRRHHModels> obtenerSubtipoSoliRrhh(string tipo)
        {
            var db = new ApplicationDbContext();
            var subtip = db.tb_SubtipoSolicitudRRHH
                .Include(x => x.tipoSolicitud)
                .OrderBy(x=>x.nomSubtipoSolicitud)
                .Where(x => x.tipoSolicitud.idTipoSolicitudRrhh == tipo).ToList();
            return subtip;
        }
        public SubtipoSolicitudRRHHModels obtenerSubtipoSoliRrhhId(string id)
        {
            var db = new ApplicationDbContext();
            var subtip = db.tb_SubtipoSolicitudRRHH
                .Include(x => x.tipoSolicitud)
                .Where(x => x.idSubTipoSolicitudRrhh == id).FirstOrDefault();
            return subtip;
        }
